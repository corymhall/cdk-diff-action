...
const resolveCredentialSource = (credentialSource, profileName, logger) => {
    const sourceProvidersMap = {
        EcsContainer: async (options) => {
            const { fromHttp } = await __webpack_require__.e(/* import() */ 395).then(__webpack_require__.bind(__webpack_require__, 18395));
        ...
        Environment: async (options) => {
            ...
            const { fromEnv } = await Promise.resolve(/* import() */).then(__webpack_require__.t.bind(__webpack_require__, 56648, 19));
...
const resolveAssumeRoleCredentials = async (profileName, profiles, options, callerClientConfig, visitedProfiles = {}, resolveProfileData) => {
    ...
    if (!options.roleAssumer) {
        const { getDefaultRoleAssumer } = await __webpack_require__.e(/* import() */ 794).then(__webpack_require__.t.bind(__webpack_require__, 15794, 23));
...
const resolveProcessCredentials = async (options, profile) => __webpack_require__.e(/* import() */ 718).then(__webpack_require__.t.bind(__webpack_require__, 80718, 19)).then(({ fromProcess }) => fromProcess({
    ...options,
    profile,
...
const resolveSsoCredentials = async (profile, profileData, options = {}, callerClientConfig) => {
    const { fromSSO } = await __webpack_require__.e(/* import() */ 756).then(__webpack_require__.t.bind(__webpack_require__, 8756, 19));
    return fromSSO({
        profile,
...
const isWebIdentityProfile = (arg) => Boolean(arg) &&
    ...
    typeof arg.role_arn === "string" &&
    ["undefined", "string"].indexOf(typeof arg.role_session_name) > -1;
const resolveWebIdentityCredentials = async (profile, options, callerClientConfig) => Promise.all(/* import() */[__webpack_require__.e(794), __webpack_require__.e(550)]).then(__webpack_require__.t.bind(__webpack_require__, 36550, 23)).then(({ fromTokenFile }) => fromTokenFile({
...
class LoginCredentialsFetcher {
    ...
    async refresh(token) {
        const { SigninClient, CreateOAuth2TokenCommand } = await __webpack_require__.e(/* import() */ 652).then(__webpack_require__.t.bind(__webpack_require__, 66652, 23));
